...
async function peopleCardContents(data) {
    ...
    html += `<p><strong>Details:</strong> gender ${data.gender}, age ${data.age}</p>, eye color ${data.eye_color}, hair color ${data.hair_color};`
...
async function locationCardContents(data){
    ...
    let html = `<h2>${data.name}</h2>`;
    html += `<p><strong>Details:</strong> climate ${data.climate}, terrain ${data.terrain}</p>, surface water ${data.surface_water}%</p>`
